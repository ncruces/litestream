...
func (f *liteFile) buildIndex(ctx context.Context, syncTime time.Time) error {
	...
	infos, err := CalcRestorePlan(ctx, f.db.client, 0, syncTime, f.db.opts.Logger)
	if err != nil {
...
func (d *liteDB) buildIndex(ctx context.Context) error {
	...
	// Build the index from scratch from a Litestream restore plan.
	infos, err := CalcRestorePlan(ctx, d.client, 0, time.Time{}, d.opts.Logger)
	if err != nil {
		return fmt.Errorf("calc restore plan: %w", err)
	}
...
func (d *liteDB) pollReplica(ctx context.Context) (*pageIndex, ltx.TXID, error) {
	...
	for level := range []int{0, 1, SnapshotLevel} {
...
func fetchPageIndex(
	...
	client ReplicaClient, info *ltx.FileInfo) (*pageIndex, error) {

	idx, err := FetchPageIndex(ctx, client, info)
	if err != nil {
...
type levelTXIDs = [SnapshotLevel + 1]ltx.TXID
